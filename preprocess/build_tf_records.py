...
tf.app.flags.DEFINE_integer('train_shards', 2,  # for N_TRAIN_SAMPLES / N_SAMPLES_PER_TRAIN_SHARD
                            'Number of shards in training TFRecord files.')#shard numbers in the whole dataset
tf.app.flags.DEFINE_integer('validation_shards', 2,  # N_VALIDATION_SAMPLES / N_SAMPLES_PER_VALIDATION_SHARD
                            'Number of shards in validation TFRecord files.')

tf.app.flags.DEFINE_integer('num_train_threads', 2,
...
def _process_image_files_batch(coder, thread_index, ranges, name, file_names, labels, num_shards):
    ...
    #shard_ranges divide a thread shard num(num_trainset/threads) into num_shards_per_thread shares: ranges
...
def _process_image_files(name, file_names, labels, num_shards, num_threads):
    ...
    Args:
      name: string, unique identifier specifying the data set, prefix of tf-record train or validation
    ...
    for i in range(len(spacing) - 1):
        ranges.append([spacing[i], spacing[i + 1]])  # images are divided into thread_num parts
    ...
    for thread_index in range(len(ranges)):
        ...
        t = threading.Thread(target=_process_image_files_batch, args=args) #create a thread
        t.start()
        threads.append(t)
...
def _find_image_files(data_dir):
    ...
    unique_labels = ['Extracted_Negative_Patches', 'Extracted_Positive_Patches']
    ...
    for label in unique_labels:
        png_file_path = '%s/%s/*' % (data_dir, label)
        matching_files = tf.gfile.Glob(png_file_path)
        #file_names and labels is loaded 1st negative 2nd positive
        labels.extend([label_index] * len(matching_files))#0 is negative 1 is positive
        file_names.extend(matching_files)

        print('Finished finding files in %d of %d examples.' % (label_index, len(labels)))
        label_index += 1
    ...
    labels = [labels[i] for i in shuffled_index] #shuffle file_names and labels but keep coincident with both
...
def _process_dataset(name, directory, num_shards, num_threads):
    ...
    Args:
      ...
      num_shards: integer number of shards for this data set. data shard
    '''
