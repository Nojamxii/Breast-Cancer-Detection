...
import numpy as np
import utils
from ops.extractor import PatchExtractor
...
def extract_positive_patches_from_tumor_wsi(wsi_ops, patch_extractor, patch_index, augmentation=False):
    ...
    patch_save_dir = utils.PATCHES_VALIDATION_AUG_POSITIVE_PATH if augmentation else utils.EXTRACTED_PATCHES_POSITIVE_PATH
...
def extract_patches(ops, pe):
    ...
    patch_index_negative = extract_negative_patches_from_normal_wsi(ops, pe, patch_index_negative)
    # patch_index_positive = extract_positive_patches_from_tumor_wsi(ops, pe, patch_index_positive)
...
if __name__ == '__main__':
    #Extract patches from original WSI images
